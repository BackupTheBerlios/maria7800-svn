...
namespace Maria.Core {
	public sealed class TIATables {
		public static readonly TIA.CxPairFlags[] CollisionMask = BuildCollisionMaskTable();
		...
		public static readonly int[, ,] PxPosResetWhen = BuildPxPosResetWhenTable();
		...
		private static uint[,] BuildPFMaskTable() {
			uint[,] tabl = new uint[2, 160];
			for (int i = 0; i < 20; i++) {
				uint mask = 0;
				if (i < 4) {
					mask = (uint)(1 << i);
		...
		private static bool[,] BuildBLMaskTable() {
			bool[,] tabl = new bool[4, 160];
		...
		private static bool[, ,] BuildMxMaskTable() {
			bool[, ,] tabl = new bool[4, 8, 160];
			for (int i = 0; i < 4; i++) {
				for (int j = 0; j < 8; j++) {
					for (int k = 0; k < 160; k++) {
						tabl[i, j, k] = false;
		...
		private static byte[, ,] BuildPxMaskTable() {
			...
			for (int nusiz = 0; nusiz < 8; nusiz++) {
				for (int i = 0; i < 160; i++) {
					...
					// 6: 3 copies medium
					// 7: quad sized player
					tabl[0, nusiz, i] = tabl[1, nusiz, i] = 0;
					if (nusiz >= 0 && nusiz <= 4 || nusiz == 6) {
						if (i >= 0 && i < 8) {
							tabl[0, nusiz, i] = (byte)(1 << (7 - i));
						}
					}
					if (nusiz == 1 || nusiz == 3) {
						if (i >= 16 && i < 24) {
							tabl[0, nusiz, i] = (byte)(1 << (23 - i));
							tabl[1, nusiz, i] = (byte)(1 << (23 - i));
						}
					}
					if (nusiz == 2 || nusiz == 3 || nusiz == 6) {
						if (i >= 32 && i < 40) {
							tabl[0, nusiz, i] = (byte)(1 << (39 - i));
							tabl[1, nusiz, i] = (byte)(1 << (39 - i));
						}
					}
					if (nusiz == 4 || nusiz == 6) {
						if (i >= 64 && i < 72) {
							tabl[0, nusiz, i] = (byte)(1 << (71 - i));
							tabl[1, nusiz, i] = (byte)(1 << (71 - i));
						}
					}
					if (nusiz == 5) {
						if (i >= 0 && i < 16) {
							tabl[0, nusiz, i] = (byte)(1 << ((15 - i) >> 1));
						}
					}
					if (nusiz == 7) {
						if (i >= 0 && i < 32) {
							tabl[0, nusiz, i] = (byte)(1 << ((31 - i) >> 2));
						}
					}
				}
				int shift = 1;
				if (nusiz == 5) { // double size player
					shift <<= 1;
				}
				if (nusiz == 7) { // quad size player
					shift <<= 1;
				}
				while (shift-- > 0) {
					for (int i = 159; i > 0; i--) {
		...
		private static byte[] BuildGRPReflectTable() {
			byte[] tabl = new byte[256];
			for (int i = 0; i < 256; i++) {
				byte s = (byte)i;
				byte r = 0;
				for (int j = 0; j < 8; j++) {
		...
		private static int[, ,] BuildPxPosResetWhenTable() {
			int[, ,] tabl = new int[8, 160, 160];
			for (int type = 0; type < 8; type++) {
				for (int oldpos = 0; oldpos < 160; oldpos++) {
					for (int newpos = 0; newpos < 160; newpos++) {
						tabl[type, oldpos, newpos] = 0;
					}
					for (int newpos = 0; newpos < 160 + 72 + 5; newpos++) {
						if (type == 0) {
							if (newpos >= (oldpos + 0) && newpos < (oldpos + 4)) {
								tabl[type, oldpos, newpos % 160] = -1;
							}
							if (newpos >= (oldpos + 4) && newpos < (oldpos + 4 + 8)) {
								tabl[type, oldpos, newpos % 160] = 1;
							}
						}
						if (type == 1) {
							...
							else if (newpos >= (oldpos + 16) && newpos < (oldpos + 16 + 4)) {
								tabl[type, oldpos, newpos % 160] = -1;
							}
							if (newpos >= (oldpos + 4) && newpos < (oldpos + 4 + 8)) {
								tabl[type, oldpos, newpos % 160] = 1;
							}
							else if (newpos >= (oldpos + 16 + 4) && newpos < (oldpos + 16 + 4 + 8)) {
								tabl[type, oldpos, newpos % 160] = 1;
							}
						}
						if (type == 2) {
							...
							else if (newpos >= (oldpos + 32) && newpos < (oldpos + 32 + 4)) {
								tabl[type, oldpos, newpos % 160] = -1;
							}
							if (newpos >= (oldpos + 4) && newpos < (oldpos + 4 + 8)) {
								tabl[type, oldpos, newpos % 160] = 1;
							}
							else if (newpos >= (oldpos + 32 + 4) && newpos < (oldpos + 32 + 4 + 8)) {
								tabl[type, oldpos, newpos % 160] = 1;
							}
						}
						if (type == 3) {
						...
						if (type == 4) {
							...
							else if (newpos >= (oldpos + 64) && newpos < (oldpos + 64 + 4)) {
								tabl[type, oldpos, newpos % 160] = -1;
							}
							if (newpos >= (oldpos + 4) && newpos < (oldpos + 4 + 8)) {
								tabl[type, oldpos, newpos % 160] = 1;
							}
							else if (newpos >= (oldpos + 64 + 4) && newpos < (oldpos + 64 + 4 + 8)) {
								tabl[type, oldpos, newpos % 160] = 1;
							}
						}
						if (type == 5) {
							...
							if (newpos >= (oldpos + 4) && newpos < (oldpos + 4 + 16)) {
								tabl[type, oldpos, newpos % 160] = 1;
							}
						}
						if (type == 6) {
						...
						if (type == 7) {
							...
							if (newpos >= (oldpos + 4) && newpos < (oldpos + 4 + 32)) {
								tabl[type, oldpos, newpos % 160] = 1;
							}
						}
					}
		...
		private static TIA.CxPairFlags[] BuildCollisionMaskTable() {
			TIA.CxPairFlags[] tabl = new TIA.CxPairFlags[64];
			for (int i = 0; i < 64; i++) {
				tabl[i] = 0;
				if (tstCx(i, TIA.CxFlags.M0, TIA.CxFlags.P1)) { tabl[i] |= TIA.CxPairFlags.M0P1; }
				if (tstCx(i, TIA.CxFlags.M0, TIA.CxFlags.P0)) { tabl[i] |= TIA.CxPairFlags.M0P0; }
				if (tstCx(i, TIA.CxFlags.M1, TIA.CxFlags.P0)) { tabl[i] |= TIA.CxPairFlags.M1P0; }
				if (tstCx(i, TIA.CxFlags.M1, TIA.CxFlags.P1)) { tabl[i] |= TIA.CxPairFlags.M1P1; }
				if (tstCx(i, TIA.CxFlags.P0, TIA.CxFlags.PF)) { tabl[i] |= TIA.CxPairFlags.P0PF; }
				if (tstCx(i, TIA.CxFlags.P0, TIA.CxFlags.BL)) { tabl[i] |= TIA.CxPairFlags.P0BL; }
				if (tstCx(i, TIA.CxFlags.P1, TIA.CxFlags.PF)) { tabl[i] |= TIA.CxPairFlags.P1PF; }
				if (tstCx(i, TIA.CxFlags.P1, TIA.CxFlags.BL)) { tabl[i] |= TIA.CxPairFlags.P1BL; }
				if (tstCx(i, TIA.CxFlags.M0, TIA.CxFlags.PF)) { tabl[i] |= TIA.CxPairFlags.M0PF; }
				if (tstCx(i, TIA.CxFlags.M0, TIA.CxFlags.BL)) { tabl[i] |= TIA.CxPairFlags.M0BL; }
				if (tstCx(i, TIA.CxFlags.M1, TIA.CxFlags.PF)) { tabl[i] |= TIA.CxPairFlags.M1PF; }
				if (tstCx(i, TIA.CxFlags.M1, TIA.CxFlags.BL)) { tabl[i] |= TIA.CxPairFlags.M1BL; }
				if (tstCx(i, TIA.CxFlags.BL, TIA.CxFlags.PF)) { tabl[i] |= TIA.CxPairFlags.BLPF; }
				if (tstCx(i, TIA.CxFlags.P0, TIA.CxFlags.P1)) { tabl[i] |= TIA.CxPairFlags.P0P1; }
				if (tstCx(i, TIA.CxFlags.M0, TIA.CxFlags.M1)) { tabl[i] |= TIA.CxPairFlags.M0M1; }
			}
			return tabl;
		}
	}
