...
namespace EMU7800
{
	...
	internal sealed class Rectangulator
	{
				...
				int scanline, int hposStart, int updateClocks)
		{
			...
			if (scanline < ClipTop || scanline >= FrameBufferSize.Height
					|| scanline >= ClipBottom)
			{
				return;
			}
		...
		{
			int xfactor = ViewPortSize.Width / (FrameBufferSize.Width / PixelAspectXRatio);
			int yfactor = ViewPortSize.Height / ClipHeight;
			int minfactor = xfactor <= yfactor ? xfactor : yfactor;

			PixelSize = new Size(PixelAspectXRatio * minfactor, minfactor);

			CenteringOffset = new Size();
			CenteringOffset.Width = (ViewPortSize.Width - FrameBufferSize.Width * PixelSize.Width) >> 1;
			CenteringOffset.Height = (ViewPortSize.Height - ClipHeight * PixelSize.Height) >> 1;
