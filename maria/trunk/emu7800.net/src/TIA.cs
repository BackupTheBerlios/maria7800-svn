...
namespace EMU7800
{
	...
	public class TIA : IDevice, IDeserializationCallback
	{
		...
		TIASound TIASound;
		byte[] ScanlineBuffer = new byte[160];
		int FBindex;

		Machine M;
		AddressSpace Mem;

		byte[] RegW = new byte[0x40];

		ulong FrameStartClock;
		ulong FrameStopClock;
		ulong FrameLastClock;
		ulong LastHMOVEClock;
		int RemainingScanlineClocks;
		...
		int P0pos, P0type, P0suppress;
		int P1pos, P1type, P1suppress;
		int M0pos, M0size, M0type;
		int M1pos, M1size, M1type;

		bool LRHBOn, LateHMOVEBlankOn;
		int HMOVECounter;
		HMOVERegister HMRP0, HMRP1, HMRM0, HMRM1, HMRBL;

		bool vblankon, scoreon, pfpriority;
		bool p0on, p1on, m0on, m1on, blon;
		byte colubk, colupf, colup0, colup1;

		bool DumpEnabled;
		ulong DumpDisabledCycle;

		CxPairFlags Collisions;
		...
		public void StartFrame()
		{
			...
			FrameStopClock = FrameStartClock + (ulong)(228 * M.Scanlines);
		...
		byte DumpedInputPort(int resistance)
		{
			...
			else
			{
				...
				ulong needed = (ulong)(charge_time * M.Scanlines * 228 * M.FrameHZ / 3);
		...
		void poke(ushort addr, byte data)
		{
			...
			// FIXME: Unsure if this has any meaningful effect
		...
		void opVSYNC(ushort addr, byte data)
		{
			...
			if ((data & 0x02) == 0)
			{
				bool atBottomHalf = (int)((Clock - FrameStartClock) / 228) > (M.Scanlines >> 1);
		...
		void opRESP0(ushort addr, byte data)
		{
			...
			// FIXME: Positioning cheat for Activision's Grand Prix
		...
		static int[] BuildPokeDelayTable()
		{
			...
			// FIXME: Unsure if any of these are really necessary
			//t[VBLANK] = 1;
			//t[ENABL] = 1;
			//t[ENAM0] = t[ENAM1] = 1;
