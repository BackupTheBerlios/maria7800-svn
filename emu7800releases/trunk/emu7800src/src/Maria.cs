...
namespace EMU7800 
{
	...
	public sealed class Maria : IDevice 
	{
		...
		byte[] ScanlineBuffer = new byte[320];
		byte[] LineRAM = new byte[512];
		byte[,] MariaPalette = new byte[8,4];
		byte[] Registers = new byte[0x40];
		...
		public void DoScanline(int scanline) 
		{
			...
			if (M.H != null) 
			{
				...
				M.H.UpdateDisplay(ScanlineBuffer, Scanline + 1, 0, 320);
			}
		}
